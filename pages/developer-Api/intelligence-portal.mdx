...
<a href="https://hushh.ai/intelligence-portal" target="_blank" rel="noopener noreferrer">hushh.ai/intelligence-portal</a>
...
curl -X POST "https://hushh.ai/api/a2a/public" \
  ...
  -d "{\"text\":\"Can you provide me with a detailed JSON profile of John Doe having email john.doe@example.com and phone +1 4085551234. The output should strictly be in JSON format.\"}"
```
...
curl -X POST "https://hushh.ai/api/a2a/hushh" \
  -H "Content-Type: application/json" \
  -d "{\"text\":\"Can you fetch all the details of the user having email john.doe@example.com and phone +1 4085551234? Return JSON.\"}"
```
...
curl -X POST "https://hushh.ai/api/a2a/brand" \
  -H "Content-Type: application/json" \
  -d "{\"text\":\"Create a JSON profile for John Doe (email john.doe@example.com, phone +1 4085551234) and summarize top preferences and intents.\"}"
```
